...
private:
	string getLCS(int **m, int i, int j, string s1, string s2) {
		...
		if (s1[i] == s2[j])
			return getLCS(m, i - 1, j - 1, s1, s2) + s1[i];
	...
	set<string> getAllLCS(int **m, int i, int j, string s1, string s2,
		...
		if (s1[i] == s2[j]) {
			set<string> v1 = getAllLCS(m, i - 1, j - 1, s1, s2, s1[i] + s);
...
public:
	void run() {
		...
		cout << "LCS: " << getLCS(m, s1.size() - 1, s2.size() - 1, s1, s2)
				<< endl;

		set<string> v = getAllLCS(m, s1.size() - 1, s2.size() - 1, s1, s2, "");
